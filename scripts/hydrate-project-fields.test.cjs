...
function runSelfTests() {
  ...
  assert.equal(metadata.explicitKeys.has("repo"), true);
  ...
  const fields = [
    ...
    },
    {
      ...
      name: "Repo",
      options: [
        { id: "o-github", name: ".github" },
        { id: "o-control-plane-api", name: "control-plane-api" },
        { id: "o-ai-execution-service", name: "ai-execution-service" },
        { id: "o-arbiter-console", name: "arbiter-console" },
        { id: "o-arbiter-site", name: "arbiter-site" },
        { id: "o-company", name: "company" },
        { id: "o-security", name: "security" },
      ],
    },
    { id: "f-phase", name: "Phase", options: [{ id: "o-mvp", name: "mvp" }] },
    { id: "f-validation", name: "Validation Command" },
    {
      id: "f-workstream",
      name: "Workstream",
      options: [
  ...
  const metadataMissing = { found: false, values: {}, explicitKeys: new Set(), warnings: [], unknownKeys: [] };
  ...
  for (const repoValue of ["security", "arbiter-systems/security"]) {
    plan = planHydration(
      { found: true, values: { repo: repoValue }, explicitKeys: new Set(["repo"]), warnings: [], unknownKeys: [] },
      { priority: null, status: null, warnings: [] },
      new Map(),
      fields,
    );
    assert.equal(
      plan.operations.some((op) => op.key === "repo" && op.value === "security" && op.optionId === "o-security" && op.source === "metadata"),
      true,
    );
    assert.equal(plan.errors.length, 0);
  }
  ...
  plan = planHydration(
    { found: true, values: { repo: "arbiter-systems/.github" }, explicitKeys: new Set(["repo"]), warnings: [], unknownKeys: [] },
    { priority: null, status: null, warnings: [] },
    new Map(),
    fields,
  );
  assert.equal(
    plan.operations.some((op) => op.key === "repo" && op.value === ".github" && op.optionId === "o-github" && op.source === "metadata"),
    true,
  );
  assert.equal(plan.errors.length, 0);
  ...
  plan = planHydration(
    { found: true, values: { workstream: "security-privacy" }, explicitKeys: new Set(["workstream"]), warnings: [], unknownKeys: [] },
    { priority: null, status: null, warnings: [] },
    new Map(),
    fields,
  );
  assert.equal(
    plan.operations.some((op) => op.key === "workstream" && op.value === "Security & Compliance" && op.optionId === "o-security-compliance"),
    true,
  );
  assert.equal(plan.errors.length, 0);

  plan = planHydration(
    { found: true, values: { phase: "mvp", validation_command: "npm test" }, explicitKeys: new Set(["phase", "validation_command"]), warnings: [], unknownKeys: [] },
    { priority: null, status: null, warnings: [] },
    new Map(),
    fields.filter((field) => field.name !== "Phase"),
  );
  assert.equal(plan.operations.some((op) => op.key === "phase"), false);
  assert.equal(plan.warnings.some((warning) => warning.includes("Optional project field for 'phase' was not found")), true);
  ...
  plan = planHydration(
    { found: true, values: { project_priority: "Critical" }, explicitKeys: new Set(["project_priority"]), warnings: [], unknownKeys: [] },
    { priority: null, status: null, warnings: [] },
    new Map(),
    fields,
  );
  assert.equal(plan.operations.some((op) => op.key === "project_priority"), false);
  assert.equal(plan.errors.some((error) => error.includes("Unknown single-select option 'Critical' for field 'Project Priority'.")), true);
